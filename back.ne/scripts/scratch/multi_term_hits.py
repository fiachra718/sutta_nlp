...
def normalize_query_term(t: str) -> str:
    t = t.replace("’", "'").replace("‘", "'").replace("“", '"').replace("”", '"')
    # keep both original and ASCII-folded forms
    ascii_fold = unicodedata.normalize("NFKD", t).encode("ascii", "ignore").decode("ascii")
    return t, ascii_fold
...
def main():
    ...
    ap.add_argument("--normalize-terms", action="store_true",
                help="Add ASCII-folded duplicates and normalize quotes in query terms.")
    args = ap.parse_args()

        # before executing SQL:
    raw_terms = args.terms
    terms = []
    if getattr(args, "normalize-terms", False):
        for t in raw_terms:
            a, b = normalize_query_term(t)
            terms.extend({a, b})  # de-dup set → list
    else:
        terms = raw_terms
